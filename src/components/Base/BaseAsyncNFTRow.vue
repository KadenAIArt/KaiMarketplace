...
onMounted(async() => {
          ...
          try{
            ...
            if(nftres.salesData.saleType !== undefined){
              ...
              if(nftres.salesData.saleType === 'fixed' && nftres.salesData.isActive === true){
                ...
                current_price.value = nftres.salesData.salePrice;
  ...
  if(current_price.value === null){ current_price.value = '-'}
  if(highest_bid.value === null){ highest_bid.value = "-" }
  if(last_sale.value === null){ last_sale.value = "-" }
...
<template>
  <tr ref="containerRef" v-motion-slide-right @click="clickNFT(props.id)" class="bg-white cursor-pointer border-b-2 dark:border-b-foreground/60 dark:bg-dark  rounded-lg hover:bg-light  dark:hover:bg-foreground">
    ...
    <th scope="row"  class="flex items-center px-6 py-5 rounded-l-lg mr-10 md:mr:0 text-gray-900 whitespace-nowrap dark:text-white">
      <div v-if="image === null"  class="skeleton rounded-box h-[110px] w-[110px] max-h-[110px] max-w-[110px] object-contain"></div>
      <img v-if="image !== null" class="h-[110px] w-[110px] max-h-[110px] max-w-[110px] object-contain rounded-lg" height="100" width="100"  :src="image || '/images/kadenai_black.svg'"  @error="$event.target.src = '/images/kadenai_black.svg'" alt="NFT">
      <div class="pl-6 ">
        <p
    ...
    <td class="px-6 py-4">
      <p
        ...
        {{current_price === null ? '&nbsp;' : current_price}} {{coin_name === null ? '&nbsp;' : coin_name}}
      </p>
    </td>
<!--    <td class="px-6 py-4">-->
<!--      <p-->
<!--          :class="highest_bid === null ? 'skeleton' : ''"-->
<!--         class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">-->
<!--        {{highest_bid === null ? '&nbsp;' : highest_bid}}-->
<!--      </p>-->
<!--    </td>-->
<!--    <td class="px-6 py-4">-->
<!--      <p-->
<!--          :class="last_sale === null ? 'skeleton' : ''"-->
<!--          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">-->
<!--        {{last_sale === null ? '&nbsp;' : last_sale}}-->
<!--      </p>-->
<!--    </td>-->
    <td class="px-6 py-4 text-right h-18 rounded-r-lg">
      <p
          :class="owner === null ? 'skeleton' : ''"
          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">
        {{owner === null ? '&nbsp;' : owner.slice(0,20)+'..'}}
      </p>
    </td>
  </tr>
</template>
