...
onMounted(async() => {
  ...
  action_type.value = "Sold";
  action_price.value = props.sale_amount;
  action_from.value = props.seller;
  action_to.value = props.buyer;

  if(action_type.value === null){ action_type.value = "-" }
  if(action_price.value === null){ action_price.value = "-" }
  if(action_from.value === null){ action_from.value = "-" }
  if(action_to.value === null){ action_to.value = "-" }
...
<template>
  <tr ref="containerRef" v-motion-slide-right class="bg-white border-b-2 dark:border-b-foreground/60 dark:bg-dark rounded-lg hover:bg-light  dark:hover:bg-foreground">
    ...
    <th scope="row"  class="flex items-center px-6 py-5 rounded-l-lg mr-10 md:mr:0 text-gray-900 whitespace-nowrap dark:text-white">
      ...
      <img v-if="image !== null" class="h-[110px] w-[110px] max-h-[110px] max-w-[110px] object-cover rounded-lg" height="100" width="100"  :src="image || '/images/kadenai_black.svg'"  @error="$event.target.src = '/images/kadenai_black.svg'" alt="NFT">
      <div class="pl-6 ">
        <p
    ...
    <td class="px-6 py-4">
      <p
          :class="action_type === null ? 'skeleton' : ''"
          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">
    ...
    <td class="px-6 py-4">
      <p
          :class="action_price === null ? 'skeleton' : ''"
          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">
        {{action_price === null ? '&nbsp;' : action_price+' KDA'}}
      </p>
    </td>
    <td class="px-6 py-4">
      <p
          :class="action_from === null ? 'skeleton' : ''"
          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">
        {{action_from === null ? '&nbsp;' : action_from.slice(0,10)+'..'}}
      </p>
    </td>
    <td class="px-6 py-4 text-right h-18 rounded-r-lg">
      <p
          :class="action_to === null ? 'skeleton' : ''"
          class="md:text-lg mt-2 font-2 text-black dark:text-light_grey">
        {{action_to === null ? '&nbsp;' : action_to.slice(0,10)+'..'}}
      </p>
    </td>
  </tr>
</template>
